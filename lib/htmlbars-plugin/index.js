...
const localHelperName = 'local-class';
const localAttrName = 'class';
const globalAttrName = 'global-class';
...
module.exports = class ClassTransformPlugin {
  ...
  determineStylesModule(meta) {
    ...
    if (name.endsWith('template')) {
      return name.replace(/template$/, 'style');
    } else if (name.includes('/templates/')) {
      return name.replace('/templates/', '/style/');
    }
  }
  ...
  transformStatement(node) {
    if (node.path.original === localHelperName) {
  ...
  transformElementNode(node) {
    let localClassAttr = utils.getAttr(node, localAttrName);
    let globalClassAttr = utils.getAttr(node, globalAttrName);
    if (!localClassAttr && !globalClassAttr) { return; }
    ...
    if (globalClassAttr) {
      let classAttr = globalClassAttr;
      utils.removeAttr(node, classAttr);
      classAttrValue = classAttr.value;

      console.log('HERE', classAttr);
      console.log('VALUE', classAttrValue);
      console.log('YO', JSON.stringify(classAttr));
  ...
  dynamicLocalPath(node) {
    let localClassPath = this.builders.path(localHelperName);
